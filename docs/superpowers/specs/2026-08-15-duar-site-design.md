# Duar product site (duar.io) — Design

> Approved 2026-08-15. A single-page marketing/product site for Duar,
...
> Duar brand. Lives in a **new repo `sidxz/duar-site`**; this spec sits in
...
- **Scope:** landing page only. Navbar/footer link *out* to the existing MkDocs
  docs (`https://docs.duar.io/`) and GitHub. No fumadocs, no blog, no
...
- **Positioning:** "the authorization layer between your IdP and your app" — bring
  your own IdP (Google / GitHub / Entra ID / any OIDC); Duar adds workspaces,
  roles, per-resource permissions, SDKs. Not pitched as an Auth0/Keycloak
  replacement; pitched as what you build *after* login.
- **Identity:** white/ink monochrome base, **Duar red `#f43737` as the single
  ...
  docs/admin. Wordmark "Duar" + " Auth" muted.
- **Type:** IBM Plex Sans (display + body) + IBM Plex Mono (eyebrows/labels/
  ...
  admin panel.
- **Hosting:** GitHub Pages project URL now (`https://sidxz.github.io/duar-site/`,
  env-driven `basePath`); flip to `duar.io` later (drop the env line +
  add `public/CNAME`; user points GoDaddy apex A records at GitHub Pages).
  `docs.duar.io` already serves the docs from this repo.
...
- Docs home `https://docs.duar.io/` · Getting started `https://docs.duar.io/getting-started/` · Security `https://docs.duar.io/security/`
- Guides: how-it-works, workspaces, authorization, service-apps, admin-panel under `https://docs.duar.io/guide/<slug>/`
- Tutorials `https://docs.duar.io/tutorial/react/`, `https://docs.duar.io/tutorial/nextjs/`; SDK docs `https://docs.duar.io/sdk/`, `https://docs.duar.io/js-sdk/`
- GitHub `https://github.com/sidxz/duar` (Issues `/issues`, Changelog `/blob/main/CHANGELOG.md`)
- PyPI `https://pypi.org/project/duar-auth/`; npm `https://www.npmjs.com/package/@duar-auth/{js,react,nextjs}`; image `ghcr.io/sidxz/duar`

## Stack & repo layout

`sidxz/duar-site` — pnpm, Next 16 App Router `output: "export"`, React 19,
...
```
duar-site/
  .github/workflows/deploy.yml     # copied from docustore; sets BASE_PATH=/duar-site
...
```
--color-brand:       #f43737   /* Duar red — the one accent */
...
"Duar" + muted " Auth". Right: `Features` (→ `#features`), `Docs ↗`
(→ `https://docs.duar.io/`), GitHub icon (→ `https://github.com/sidxz/duar`),
`Get started` (ctaPrimary → `https://docs.duar.io/getting-started/`).
...
Sub: "Keep Sign in with Google, GitHub, or Entra ID exactly as it is. Duar
...
01 Sign in with your IdP · 02 Duar verifies the IdP token and mints an authz
...
- **Bring your own IdP** — "AuthZ mode: your app logs in, Duar verifies the
  IdP token and mints an authz JWT. Proxy mode if you'd rather Duar own the
  flow." mock: `id_token ─▶ authz_jwt · workspace_role: editor`. → `https://docs.duar.io/guide/how-it-works/`
- **Workspaces & organizations** — "Multi-tenant by default. owner / admin /
  ...
  → `https://docs.duar.io/guide/workspaces/`
- **Three-tier authorization** — "Workspace roles in the JWT, RBAC actions in the
  ...
  / `can("document", id, "view")`. → `https://docs.duar.io/guide/authorization/`
- **Service-to-service** — "Service keys, realms, and m2m calls with or without a
  user in context." mock: `sk_… → realm: platform ✓`. → `https://docs.duar.io/guide/service-apps/`
...
- **FastAPI** — `Duar(...)` config, `duar.protect(app)`,
  `@app.get("/projects") … Depends(duar.require_user)` (from docs/index.md).
  Below: `pip install duar-auth`.
- **React** — `AuthzProvider` + `AuthzGuard fallback={<Login/>}` +
  `useAuthz().login("google")` (README JS section, trimmed to ~14 lines).
  Below: `npm i @duar-auth/react`.
- **Next.js** — `createDuarAuthzMiddleware({...})` middleware.ts (docs
  tutorial/nextjs). Below: `npm i @duar-auth/nextjs`.
...
`admin.duar.local`) around either `<img src>` or, when no `src`, a
...
mono role; right dark `bg-ink` `Topology` card: Browser/App ⇄ IdP; App → Duar
...
/ `Star on GitHub ↗` + mono line `docker pull ghcr.io/sidxz/duar`. Beneath,
...
`duar-auth`, `@duar-auth/js`, `/react`, `/nextjs`) · **Community**
(GitHub, Issues, Changelog); bottom bar `© {year} Duar` · `MIT License`
mono 10px.
...
- `logo.tsx` — `LogoMark` 32×32: shield outline, hexagon core with keyhole,
  ...
  ("Duar" 500 + " Auth" `text-ink/30`). `icon.svg` = hexagon+keyhole
...
`BASE_PATH: /duar-site` → `configure-pages@v5` → `upload-pages-artifact@v3`
...
`public/CNAME` = `duar.io`, set `SITE_URL` metadata to
`https://duar.io`; user points the apex A records (currently GoDaddy
...
- Serve `out/` locally under the `/duar-site` prefix; screenshot desktop
...
- In identity-service: add a root `LICENSE` (README/npm say MIT, no file); fix
  `mkdocs.yml site_url` → `https://docs.duar.io/`.
